...
@compiled
def evaluate(pos: Array) -> int:
    """Gabriel's nonterminal evaluation ported to arrays, from side-to-move view."""
    score, material = 0, 0
    ...
    for square in range(128):
        ...
        if kind == 1 and passed_pawn(pos, square, side):
            score += side * (0, 0, 10, 25, 60, 180, 500, 0)[rank]
    ...
    return int((score + material) * pos[TURN] + 4 * len(legal_moves(pos)))
...
def analyze(
    ...
    """Run instrumented search; tt_mode 0=off, 1=move hints, 2=context-safe scores."""
    pos = encode(board)
...
def get_move(fen: str, time_left_ms: int) -> str:
    ...
    if _LAST_FEN is not None:
        ...
        if board.fullmove_number < previous.fullmove_number or fen == _LAST_FEN:
            ...
            clear_tables()
    start = time.monotonic()
    budget = max(0.001, min(3.0, (time_left_ms - 100) / 40000))
    result = analyze(board, budget, prior=_HISTORY)
    ...
    _LAST_FEN = fen
    LAST_SEARCH.clear()
    LAST_SEARCH.update({key: result[key] for key in ("nodes", "depth", "tt_hits", "nps")})
