...
MOBILITY_WEIGHT = 4
PASSED_PAWN_BONUSES: tuple[int, ...] = (0, 0, 10, 25, 60, 180, 500, 0)
ADVANCED_PASSED_PAWN_RANK = 5
KING_SHIELD_BONUS = 12
KING_RING_ATTACK_PENALTY = 8
MAX_CHECK_EXTENSIONS = 1
MAX_PASSED_PAWN_EXTENSIONS = 1
WINNING_ENDGAME_MARGIN = PIECE_VALUES[chess.ROOK]
FIFTY_MOVE_URGENCY_START = 40
FIFTY_MOVE_URGENCY_WEIGHT = 4
MOP_UP_EDGE_WEIGHT = 10
MOP_UP_KING_PROXIMITY_WEIGHT = 12
MOP_UP_CONFINEMENT_WEIGHT = 12
...
NEAR_PROMOTION_MASKS: dict[chess.Color, int] = {
    chess.WHITE: chess.BB_RANK_5 | chess.BB_RANK_6 | chess.BB_RANK_7,
    chess.BLACK: chess.BB_RANK_2 | chess.BB_RANK_3 | chess.BB_RANK_4,
}
...
def is_claimable_fifty_move_draw(board: chess.Board) -> bool:
    """Match the referee's automatic fifty-move claim at the current node."""
    if board.is_fifty_moves():
        return True

    # python-chess can claim at 99 halfmoves when a legal non-zeroing move would
    # complete the count. Avoid its legal-move scan at ordinary search nodes.
    return board.halfmove_clock >= 99 and board.can_claim_fifty_moves()
...
def is_search_draw(board: chess.Board, current_repetitions: int) -> bool:
    ...
    return current_repetitions >= 3 or is_claimable_fifty_move_draw(board)
...
def pawn_advancement(square: chess.Square, side: chess.Color) -> int:
    """Return how many ranks a pawn has advanced from its own back rank."""
    rank = chess.square_rank(square)
    return rank if side == chess.WHITE else 7 - rank


def is_passed_pawn(board: chess.Board, square: chess.Square, side: chess.Color) -> bool:
    """Return whether opposing pawns cannot block this pawn on nearby files."""
    opponent_pawns = board.pieces_mask(chess.PAWN, not side)
    return not bool(opponent_pawns & PASSED_PAWN_MASKS[side][square])


def is_advanced_passed_pawn_push(board: chess.Board, move: chess.Move) -> bool:
    """Return whether a move pushes a passer onto its sixth or seventh rank."""
    piece = board.piece_at(move.from_square)

    if piece is None or piece.piece_type != chess.PAWN or move.promotion is not None:
        return False

    return (
        pawn_advancement(move.to_square, piece.color) >= ADVANCED_PASSED_PAWN_RANK
        and is_passed_pawn(board, move.to_square, piece.color)
    )


def has_near_promotion_pawn(board: chess.Board) -> bool:
    """Return whether either side has a pawn close enough to need special search."""
    return bool(
        board.pieces_mask(chess.PAWN, chess.WHITE) & NEAR_PROMOTION_MASKS[chess.WHITE]
        or board.pieces_mask(chess.PAWN, chess.BLACK) & NEAR_PROMOTION_MASKS[chess.BLACK]
    )


def captures_advanced_passed_pawn(board: chess.Board, move: chess.Move) -> bool:
    """Return whether a move removes an enemy passer near promotion."""
    ...
    return (
        pawn_advancement(move.to_square, victim.color) >= ADVANCED_PASSED_PAWN_RANK
        and is_passed_pawn(board, move.to_square, victim.color)
    )
...
def passed_pawn_bonus(board: chess.Board, side: chess.Color) -> int:
    ...
    for square in board.pieces(chess.PAWN, side):
        if not is_passed_pawn(board, square, side):
            continue

        advancement = pawn_advancement(square, side)
...
def fifty_move_clock_score(
    board: chess.Board,
    material_advantage: int,
) -> int:
    """Make an approaching fifty-move draw bad for a clearly winning side."""
    if abs(material_advantage) < WINNING_ENDGAME_MARGIN:
        return 0

    urgency = max(0, board.halfmove_clock - FIFTY_MOVE_URGENCY_START)
    penalty = urgency * FIFTY_MOVE_URGENCY_WEIGHT
    return -penalty if material_advantage > 0 else penalty


def king_escape_square_count(board: chess.Board, side: chess.Color) -> int:
    """Estimate how many unattacked adjacent squares a king can use."""
    king_square = board.king(side)

    if king_square is None:
        return 0

    candidate_squares = chess.BB_KING_ATTACKS[king_square] & ~board.occupied_co[side]
    enemy = not side
    return sum(
        not board.is_attacked_by(enemy, square)
        for square in chess.scan_forward(candidate_squares)
    )


def can_mop_up(board: chess.Board, side: chess.Color) -> bool:
    """Return whether side has a major piece against a completely bare king."""
    enemy = not side
    ...
    has_major_piece = bool(
        board.pieces_mask(chess.ROOK, side) | board.pieces_mask(chess.QUEEN, side)
    )
    enemy_has_other_piece = bool(board.occupied_co[enemy] & ~chess.BB_SQUARES[enemy_king])
    return has_major_piece and not enemy_has_other_piece


def mop_up_bonus(board: chess.Board, side: chess.Color) -> int:
    """Reward reliable progress when a rook or queen faces a bare king."""
    ...
    if not can_mop_up(board, side):
        return 0

    enemy_file = chess.square_file(enemy_king)
    enemy_rank = chess.square_rank(enemy_king)
    doubled_edge_distance = abs(2 * enemy_file - 7) + abs(2 * enemy_rank - 7)
    king_distance = chess.square_distance(own_king, enemy_king)
    escape_squares = king_escape_square_count(board, enemy)

    return (
        doubled_edge_distance * MOP_UP_EDGE_WEIGHT
        + (7 - king_distance) * MOP_UP_KING_PROXIMITY_WEIGHT
        + (8 - escape_squares) * MOP_UP_CONFINEMENT_WEIGHT
    )


def mop_up_score(board: chess.Board, side: chess.Color) -> int:
    """Return the side-relative score for converting against a bare king."""
    return mop_up_bonus(board, side) - mop_up_bonus(board, not side)
...
def position_score(board: chess.Board, side: chess.Color) -> int:
    ...
    return (
        ...
        + fifty_move_clock_score(board, material)
        + mop_up_score(board, side)
    )
...
def ordered_moves(board: chess.Board) -> list[chess.Move]:
    ...
    prioritize_checks = can_mop_up(board, board.turn)
    prioritize_passers = has_near_promotion_pawn(board)

    if not prioritize_checks and not prioritize_passers:
        moves.sort(key=lambda move: move_order_key(board, move), reverse=True)
        return moves

    moves.sort(
        key=lambda move: (
            int(prioritize_checks and board.gives_check(move)),
            0 if move.promotion is None else PIECE_VALUES.get(move.promotion, 0),
            int(prioritize_passers and captures_advanced_passed_pawn(board, move)),
            *move_order_key(board, move),
            int(prioritize_passers and is_advanced_passed_pawn_push(board, move)),
        ),
        reverse=True,
    )
    return moves
...
def tactical_move_order_key(
    ...
    return (
        promotion_value,
        int(captures_advanced_passed_pawn(board, move)),
        capture_flag,
        int(is_advanced_passed_pawn_push(board, move)),
        victim_value,
        attacker_score,
    )
...
def ordered_tactical_moves(board: chess.Board) -> list[chess.Move]:
    """Return captures, promotions, and dangerous passed-pawn pushes in order."""
    if not has_near_promotion_pawn(board):
        moves = [
            move
    ...
    moves = [
        ...
        or is_advanced_passed_pawn_push(board, move)
    ]
    moves.sort(
...
def alpha_beta(
    ...
    passed_pawn_extensions_remaining: int,
    position_counts: PositionCounts,
    current_repetitions: int,
) -> int:
    ...
    if maximizing:
        value = -MATE_SCORE
        can_extend_passed_pawn = (
            passed_pawn_extensions_remaining > 0 and has_near_promotion_pawn(board)
        )

        for move in moves:
            extends_passed_pawn = (
                can_extend_passed_pawn
                and is_advanced_passed_pawn_push(board, move)
            )
            board.push(move)
            key, repetitions = record_search_position(board, position_counts)
            ...
            try:
                ...
                extends_search = extends_check or extends_passed_pawn
                score = alpha_beta(
                    board,
                    depth - 1 + int(extends_search),
                    alpha,
                    beta,
                    False,
                    mover,
                    ...
                    passed_pawn_extensions_remaining - int(extends_passed_pawn),
                    position_counts,
                    repetitions,
                )
            finally:
    ...
    for move in moves:
        extends_passed_pawn = (
            can_extend_passed_pawn and is_advanced_passed_pawn_push(board, move)
        )
        board.push(move)
        key, repetitions = record_search_position(board, position_counts)
...
def search_at_depth(
    ...
    can_extend_passed_pawn = (
        MAX_PASSED_PAWN_EXTENSIONS > 0 and has_near_promotion_pawn(board)
    )
        ...
        try:
            if is_search_draw(board, repetitions):
                ...
                score = alpha_beta(
                    ...
                    MAX_PASSED_PAWN_EXTENSIONS - int(extends_passed_pawn),
                    position_counts,
                    repetitions,
                )
        finally:
